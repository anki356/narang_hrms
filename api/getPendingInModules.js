...
const getPendingInModules = (req, res, next) => {
    ...
    database.query("Select * from roles where id=" + role_id, (err, result) => {
        ...
        if (allowed_roles.includes(result[0].role_name)) {
            if(req.query.table_name!=='grades'&&req.query.table_name!=='attendance_approval'&&req.query.table_name!=='timing_approval' )  
           {
         database.query("select  count(distinct "+req.query.table_name+".id) as count_id from "+req.query.table_name+" left join employees on employees.id="+req.query.table_name+".id where "+req.query.table_name+".status='Pending' and employees.status=1", (err, result, fields) => {
            ...
            database.query("select count(distinct "+req.query.table_name+".id) as count_id from "+req.query.table_name+" left join employees on employees.id="+req.query.table_name+".id left join job_details on job_details.id=employees.job_details_id  where date>="+mysql.escape(from_date.format("YYYY-MM-DD"))+" and date<="+mysql.escape(to_date.format("YYYY-MM-DD"))+" and role_id in(3,10,16,17) and employees.status=1"  , (err, result, fields) => {
              database.query("select count(distinct employees.id) as count_id from employees  left join job_details on job_details.id=employees.job_details_id  where  role_id in(3,10,16,17) and employees.status=1"  , (err, employeesResult, fields) => { 
                console.log("employeesResult",result[0])
