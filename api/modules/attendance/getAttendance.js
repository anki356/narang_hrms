        ...
        database.query("select role_id ,permissions.name from permission_roles left join permissions on permissions.id=permission_roles.permission_id where name='Attendance'",(
          ...
          allowed_roles=allowed_roles.map((data)=>{
            ...
            if (allowed_roles.includes(role_id)) {
                let queryString="SELECT file_upload.name as photo,attendance.id as attendance_id,job_details.*,store_departments.name as store_department_name, roles.role_name as role_name,employees.id as employee_id,employees.employee_id as empID,employees.name as employee_name,floors.name AS floor_name,locations.name as location_name ,attendance.status,attendance.check_in_datetime as datetime FROM attendance  LEFT JOIN employees ON employees.id=attendance.employee_id left JOIN job_details ON employees.job_details_id=job_details.id left JOIN floors ON job_details.floor_id=floors.id left join roles on job_details.role_id=roles.id left join file_upload on file_upload.id=employees.photo_id left join locations on locations.id=job_details.location_id left join store_departments on store_departments.id=job_details.store_department_id where attendance.check_in_datetime>=" + mysql.escape(req.query.from_date) + " and attendance.check_in_datetime< " +mysql.escape(req.query.to_date)  +" and employees.status=1"  
                if(result[0].role_name.split(" ")[0]==='Floor'){
                    ...
                    if(req.query.floor_name){
                       ...
                       if(req.query.offset){
                        ...
                        database.query(queryString , (err, attendanceResult, fields) => {
                            console.log(queryString,attendanceResult)
                            res.send(attendanceResult) 
                        })
