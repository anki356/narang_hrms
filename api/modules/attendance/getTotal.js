...
console.log(err)
            let queryString="Select count(attendance.id) as count_id from attendance left join employees on employees.id=attendance.employee_id left join job_details on job_details.id=employees.job_details_id where check_in_datetime>="+mysql.escape(req.query.from_date)+"and check_in_datetime<"+mysql.escape(req.query.to_date)+" and status="+req.query.status+" and job_details.head_employee_id="+employeesResult[0].id
            ...
            let queryString="Select count(id) as count_id from attendance where check_in_datetime>="+mysql.escape(req.query.from_date)+"and check_in_datetime<"+mysql.escape(req.query.to_date)+" and status="+req.query.status
            if(req.query.role_name){
