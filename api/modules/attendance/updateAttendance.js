            ...
            database.query("Insert into file_upload (type,name) values ('attendance_image',"+mysql.escape(req.file.filename)+")",(err,fileResult,fields)=>{ 
                ...
                database.query("Update attendance_requests set status="+mysql.escape(req.body.approval_status)+" where attendance_id="+req.params.id, (err, attendanceRequestResult, fields) => {
                   console.log(err)
                    ...
                    database.query("Update attendance set check_in_datetime="+req.body.date_time+",status="+req.body.status+",approval_document_id="+fileResult.insertId+", status_by_id ="+ role_id+",status_date=current_timestamp(),no_of_shifts="+req.body.no_of_shifts+" where id= "+req.params.id, (err, attendanceResult, fields) => {
                    ...
                    database.query("Update attendance_requests set status="+mysql.escape(req.body.approval_status)+" where attendance_id="+req.params.id, (err, attendanceRequestResult, fields) => {
                    database.query("Update attendance set  status_by_id ="+ role_id+",status_date=current_timestamp() where id= "+req.params.id, (err, attendanceResult, fields) => {
                        console.log(err)
                        res.json({"attendanceResult":attendanceResult,"attendanceRequestResult":attendanceRequestResult}) 
