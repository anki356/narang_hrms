                ...
                if(req.body.file){
                    ...
                    database.query("Update attendance_requests set status="+mysql.escape(req.body.approval_status)+" where attendance_id="+req.params.id, (err, attendanceRequestResult, fields) => {
                        console.log(err)
                         ...
                         database.query("Update attendance set check_in_datetime="+mysql.escape(req.body.date_time)+",status="+mysql.escape(req.body.status)+", status_by_id ="+ role_id+",status_date=current_timestamp(),no_of_shifts="+req.body.no_of_shifts+" where id= "+req.params.id, (err, attendanceResult, fields) => {
                             console.log(err)
                             res.json({"attendanceResult":attendanceResult,"attendanceRequestResult":attendanceRequestResult}) 
