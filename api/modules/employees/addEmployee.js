        ...
        if (allowed_roles.includes(result[0].role_name)) {
                    database.query("Insert into file_upload (type,name,created_on) values('photo'," + mysql.escape(req.files.photo[0].filename) + ",current_timestamp())", (err, fileUploadResult, fields) => {
                        ...
                        database.query("select count(id) as count_id from employees", (err, countEmployeeId, fields) => {
                           ...
                           database.query("Insert into employees(name,father_name,phone,emergency_number,employee_id,email_address,dob,gender,marital_status,qualification,local_address,permanent_address,aadhar_no,pan_no,photo_id,job_details_id,bank_details_id,uan_no,fine_management,min_wages_as_per_rule,type) values(" + req.body.name + "," + req.body.father_name + "," + req.body.phone + "," + req.body.emergency_number + "," + countId + "," + req.body.email_address + "," + req.body.dob + "," + req.body.gender + "," + req.body.marital_status + "," + req.body.qualification + "," + req.body.local_address + "," + req.body.permanent_address + "," + req.body.aadhar_no + "," + mysql.escape(req.body.pan_no) + "," + photo_id+","+jobDetailsResult.insertId+","+bankDetailsResult.insertId+","+req.body.uan_no+","+req.body.fine_management+","+req.body.min_wages_as_per_rule+","+mysql.escape(req.body.type)+")", (err, employeesResult, fields) => {
                            console.log(err)  
