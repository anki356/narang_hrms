...
const addGrades = (from_date,to_date) => {
           ...
           let query="select employee_id,SUM(CASE WHEN STATUS = 'Present' THEN no_of_shifts ELSE 0 END)AS attendance_count FROM attendance WHERE check_in_datetime >= "+mysql.escape(from_date)+" and check_in_datetime <="+ mysql.escape(to_date)+" GROUP BY employee_id" 
