...
const restructureLoans = (req, res, next) => {
    ...
    database.query("Select * from roles where id=" + role_id, (err, result) => {
        ...
        if (allowed_roles.includes(result[0].role_name)) {
            ...
            database.query("Update loan_repayment set amount=0 where loan_id="+req.body.loan_id+" and month="+current_month,(err,firstResult,fields)=>{
                console.log(err)
             database.query("SELECT id, MAX(id) as max FROM loan_repayment where loan_id="+req.body.loan_id,(err,secondResult,fields)=>{
               ...
               let finalAmount=thirdResult[0].amount+thirdResult[0].amount
