...
const getRoleData = (req, res, next) => {
    ...
    database.query("Select * from roles where id=" + role_id, (err, result) => {
        ...
        if (allowed_roles.includes(result[0].role_name)) {
            database.query("select departments.name as department_name,departments.id as department_id, locations.name as location_name,roles.id as id,roles.role_name as role_name ,roles_locations.id as role_location_id,hierarchy.parent_role_id as parent_role_id, locations.id as location_id,locations.name as name from roles  left join roles_locations on roles_locations.role_id=roles.id left join  locations on locations.id=roles_locations.location_id left join hierarchy on hierarchy.child_role_id=roles_locations.id left join departments on departments.id =roles.department_id where roles.id="+req.query.id, (err, roleData, fields) => {
