...
const getRoles = (req, res, next) => {
    ...
    database.query("Select * from roles where id=" + role_id, (err, result) => {
        ...
        if (allowed_roles.includes(result[0].role_name)) {
            let queryString="select roles.role_name as name,departments.name as department_name ,roles.id as id,locations.name as location_name from roles  left join locations on locations.id=roles.location_id left join departments on departments.id=roles.department_id" 
