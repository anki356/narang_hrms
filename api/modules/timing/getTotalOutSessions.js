...
const getTotalOutSessions = (req, res, next) => {
    ...
    database.query("Select * from roles where id=" + role_id, (err, result) => {
        ...
        if (allowed_roles.includes(result[0].role_name)) {
            let queryString="SELECT count(timing.id) as count_id from timing left join employees on employees.id=timing.employee_id left join job_details on job_details.id=employees.job_details_id  left join locations on locations.id=job_details.location_id where date>= "+mysql.escape(req.query.from_date)+" and date<"+mysql.escape(req.query.to_date) +" and timing.in_time is null and out_time is not null" +" and employees.status=1"
