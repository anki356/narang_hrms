...
const addTransferWithlocationId = (req, res, next) => {
    ...
    database.query("Select * from roles where id=" + role_id, (err, result) => {
        ...
        if (allowed_roles.includes(result[0].role_name)) {
            database.query("Insert Into transfer_details (status,employee_id,date,department_from,department_to,floor_id_from,floor_id_to,location_id_from,location_id_to) values ('Pending',"+req.body.employee_id+", current_timestamp(), "+req.body.department_from+","+req.body.department_to+","+req.body.floor_id_from+","+req.body.floor_id_to+","+req.body.location_id_from+","+req.body.location_id_to+")",(err, transferResult, fields) => {
                console.log(err)
