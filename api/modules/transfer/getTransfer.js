...
const getTransfer = (req, res, next) => {
    ...
    database.query("Select * from roles where id=" + role_id, (err, result) => {
        ...
        if (allowed_roles.includes(result[0].role_name)) {
            let queryString="SELECT file_upload.name as photo,store_departments.name as department_from_name,store_dep.name as store_dep_name,floors.name as floor_name, transfer_details.*,employees.name as employee_name,employees.employee_id as empID from transfer_details left join employees on employees.id=transfer_details.employee_id left join store_departments on store_departments.id=transfer_details.department_from left join store_departments as store_dep on store_dep.id=transfer_details.department_to left join file_upload on file_upload.id=employees.photo_id left join job_details on job_details.id=employees.job_details_id left join floors on floors.id =job_details.floor_id left join stores on stores.id=job_details.store_id left join roles on roles.id=job_details.role_id where date>="+mysql.escape(req.query.from_date) +" and date<"+mysql.escape(req.query.to_date)
