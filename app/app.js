...
cron.schedule(" * 25 13 * * *", markAbsent, {
  scheduled: true,
...
const uploadAttendance =require("../api/modules/attendance/uploadAttendance")
...
database.query("Select * from cron_job_timestamps where name='mark_absent' and timestamp>="+mysql.escape(moment().set('h',0).set('m',0).set('s',0).format("YYYY-MM-DD HH:mm:ss"))+" and timestamp<="+mysql.escape(moment().set('h',0).set('m',0).set('s',0).add(1,'d').format("YYYY-MM-DD HH:mm:ss")),async(err,result)=>{
  if(result.length===0){
   await  markAbsent()
  }
})
let from_date_this_month = moment()
.startOf("month").add(1,'d')
.format("YYYY-MM-DD");
let to_date_this_month = moment()
.endOf("month").add(1,'d')
.format("YYYY-MM-DD");
database.query("Select * from cron_job_timestamps where name='getSalaryDetails' and timestamp>="+mysql.escape(from_date_this_month)+" and timestamp<"+mysql.escape(to_date_this_month),async(err,result)=>{
  if(result.length===0){ let from_date = moment()
    .subtract(1, "month")
    .startOf("month")
    .format("YYYY-MM-DD");
  let to_date = moment()
    ...
    .endOf("month").add(1,'d')
    .format("YYYY-MM-DD");
  await getSalaryDetails(from_date, to_date);
  }
})
router.get("/home", (req, res) => {
  return res.send("hello");
});
router.post('/upload-attendance',verifyAuth,upload.any(),verifyAuth,uploadAttendance)
